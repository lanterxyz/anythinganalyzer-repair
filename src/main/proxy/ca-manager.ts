...
export class CaManager {
  ...
  private issueLeafCert(hostname: string): { key: string; cert: string } {
    ...
    // BoringSSL on Android may reject signatures generated by node-forge's
    // pure-JS RSA implementation. Replace with native OpenSSL/BoringSSL signature.
    try {
      const caKeyPath = join(this.certsDir, CA_KEY_FILE);
      const caKeyPem = readFileSync(caKeyPath, "utf-8");
      const tbsAsn1 = forge.pki.certificateToAsn1(cert).value[0];
      const tbsDer = forge.asn1.toDer(tbsAsn1).getBytes();
      const nativeSign = crypto.createSign("sha256");
      nativeSign.update(Buffer.from(tbsDer, "binary"));
      const nativeSignature = nativeSign.sign({ key: caKeyPem, format: "pem", type: "pkcs1" });
      cert.signature = nativeSignature.toString("binary");
    } catch (err) {
